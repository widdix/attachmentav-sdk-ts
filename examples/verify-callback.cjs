...
api.whoamiGet().then(({tenantId}) => { // the tenantId never changes. we recommend to hard code it to avoid yet another HTTPS call.
  console.error(`Replace <TENANT_ID_PLACEHOLDER> with ${tenantId}, and uncomment this block`);
  process.exit(1);
});
